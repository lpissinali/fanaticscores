...
export interface MatchdayDoc {
  ...
  aiBriefStateHash?:  string;   // hash of live/FT state — skips API call when unchanged
}
 ...
 */
function mapFixtureToDoc(f: AFFixture, now: number): MatchData {
...
export function calcNextFetch(date: string, hasLive: boolean, hadErrors: boolean, now: number, nearKickoff = false): number {
  ...
  if (hasLive || nearKickoff) return now + 5 * 60_000;
...
export async function fetchMatchday(date: string, apiKey: string): Promise<MatchdayDoc> {
  ...
  const finFx   = whitelisted.filter(f => {
    if (['FT', 'AET', 'PEN'].includes(f.fixture.status.short)) return true;
    // Stale LIVE/HT counts as finished
    if (!LIVE_SHORT.includes(f.fixture.status.short)) return false;
  ...
  const nearKickoff = fixtures.some(f => {
    ...
    if (s !== 'NS' && s !== 'TBD') return false;
    const t = new Date(f.fixture.date).getTime();
    return Number.isFinite(t) && t < now + 30 * 60_000 && t > now - 4 * 3_600_000;
  });
